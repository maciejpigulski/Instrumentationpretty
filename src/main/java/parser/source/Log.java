...
/**
 * parser.source.Log class that mirrors the API in main Android sources.
...
public final class Log {
    ...
    static void hexDump(String tag, LogLevel level, byte[] data, int offset, int length) {
        ...
        //parser.source.Log.w(tag, "HEX DUMP: off=" + offset + ", length=" + length);
        baseAddr = 0;
        while (length != 0) {
            if (length > 16) {
                // full line
